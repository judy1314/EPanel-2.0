...
//				System.out.println(Generation[Now_Generation].Amount);
			...
			else if(System_Mode == 2)
			{
				if(Mode_Change == 1)
				{
					setBounds(0,0,Outline_x-(M_block_w/2)+8,Outline_y+150);
...
//					setBounds(0,0,Outline_x+8-(M_block_w/2),Outline_y+150);
	...
	//				System.out.println("k="+k);
					...
					g.drawLine(M_block_x+M_block_w+116, M_block_y-2+(M_block_l-32)/5*j, M_block_x+M_block_w+120, M_block_y-2+(M_block_l-32)/5*j);
				}
				//圖示 黑1 白0
				g.setColor(new Color(0,0,0));
				g2D.setStroke(new BasicStroke(2.0f));
...
//						System.out.println("now_y="+now_y);
						...
						if(Info_pick == 1)
						{
							...
							g.drawString(df1.format(Generation[i].X[j].Probability[k]), (int)(now_x+text_size/3), (int)(now_y+text_size*2));
						}
					}
					else
					{
...
//					System.out.println("bit_l/2="+(bit_l/2));
				...
				g22D.setFont(new Font("微軟正黑體", Font.BOLD,15));
				if(Show_P == 1)
					g22D.drawString(""+Generation[i].X[j].Fitness, M_block_x+M_block_w+5, M_block_y+now_y+((int)bit_l/2)+7);
				else
					g22D.drawString(""+Generation[i].X[j].Fitness, M_block_x+M_block_w+35, M_block_y+now_y+((int)bit_l/2)+7);
...
//				System.out.println("-----");
			...
			g22D.setFont(new Font("微軟正黑體", Font.BOLD,15));
			if(Show_P == 1)
				g22D.drawString(""+Generation[N_i].X[N_j].Fitness, M_block_x+M_block_w+5, M_block_y+now_y+((int)bit_l/2)+7);
			else
				g22D.drawString(""+Generation[N_i].X[N_j].Fitness, M_block_x+M_block_w+35, M_block_y+now_y+((int)bit_l/2)+7);
		}
		//每代顯示全部粒子(Zoom in)
		else
		{
...
/*				//顯示粒子文字
				g.setColor(new Color(255, 255, 255, 180));
				g.fillRect(430, now_y+((int)bit_l/2)-6, 140, now_y+((int)bit_l/2)+3);
				g.setColor(new Color(0,0,0));
				g2D.setFont(new Font("IrisUPC", Font.BOLD,25));
...
*/		}
		...
		for(int j=9; j>0; j--)
		{
			k += 0.1;
//				System.out.println("k="+k);
			g2D.drawString(""+df1.format(k),M_block_x+(M_block_w/2)+85,M_block_y+2+(M_block_l+R_block_w-28)/10*j);
			g.drawLine(M_block_x+(M_block_w/2)+76, M_block_y-2+(M_block_l+R_block_w-28)/10*j, M_block_x+(M_block_w/2)+80, M_block_y-2+(M_block_l+R_block_w-28)/10*j);
		}
...
//					System.out.println("x="+(now_x+35));
				...
				if(Show_P == 0)
				{
					...
					else
					{
						...
						g.setColor(new Color(255,255,255));
						g.drawRect((int)now_x+250, (int)now_y, (int)sensor_w, (int)sensor_l);
						if(Map[Now_Generation].Probability[k][j]<Map[Now_Generation].limit[k][j])
						{
//							g.setColor(new Color(255,255,255,200));
//							g.fillRect((int)now_x+250, (int)now_y, (int)sensor_w, (int)sensor_l);
							g.setColor(new Color(255,0,0));
//							g2D.setStroke(new BasicStroke(2.0f));
							g.drawLine((int)now_x+251, (int)now_y+1, (int)(now_x+251+sensor_w), (int)(now_y+sensor_l+1));
							g.drawLine((int)now_x+250, (int)(now_y+sensor_l), (int)(now_x+250+sensor_w), (int)now_y);
	...
	//							g.fillRect((int)(now_x+sensor_w/4+250-3), (int)(now_y+sensor_l/4+1), (int)sensor_w/2+8, (int)sensor_l/2);				
								g.fillRect((int)(now_x+sensor_w/8+250), (int)(now_y+sensor_l/2-(150/Map[Now_Generation].row+4)/2/*/8*3*/), (int)(sensor_w/8*6+1), (int)150/Map[Now_Generation].row+4);
							g.setColor(new Color(0,0,0));
							if((150/Map[Now_Generation].row)>3)
								g2D.drawString(df0.format(Map[Now_Generation].Probability[j][k]), (int)(now_x+250+sensor_w/2-(150/Map[Now_Generation].row)*1.3/*+text_size/2*/), (int)(now_y+sensor_l/2+(150/Map[Now_Generation].row)/2-1/*now_y+sensor_l/2+11+text_size/2*/));
...
//							g.fillRect((int)now_x+250, (int)now_y, (int)sensor_w, (int)sensor_l);
							g.setColor(new Color(255,0,0));
							g2D.setStroke(new BasicStroke(2.0f));
							g2D.drawLine((int)now_x+250, (int)now_y, (int)(now_x+250+sensor_w), (int)(now_y+sensor_l));
						}
						...
						if(Info_pick == 1)
						{
							...
							if((150/temp_Map.row)>3)
								g.fillRect((int)(now_x+sensor_w/8+250), (int)(now_y+sensor_l/2-(150/temp_Map.row+4)/2/*/8*3*/), (int)(sensor_w/8*6+1), (int)150/Map[Now_Generation].row+4);
							g.setColor(new Color(0,0,0));
							if((150/temp_Map.row)>3)
								g2D.drawString(df0.format(temp_Map.Probability[j][k]), (int)(now_x+250+sensor_w/2-(150/temp_Map.row)*1.3/*+text_size/2*/), (int)(now_y+sensor_l/2+(150/Map[Now_Generation].row)/2-1/*now_y+sensor_l/2+11+text_size/2*/));
						}
					}
				}
				now_x += sensor_w;
...
*/
			...
			if(System_Mode == 1)
			{
				...
				if (e.getSource()==Button_Video)
				{
					if(Video_mode==0)
					{
						...
						Video_x1=getX();
						Video_x2=Outline_x+8;
						Video_y1=getY();
						Video_y2=Outline_y+150;
				...
				if (e.getSource()==Button_Camera)
					Screen(getX(),getY(),Outline_x+8,Outline_y+150);
			...
			else if(System_Mode == 2)
			{
				...
				if (e.getSource()==Button_Camera)
					Screen(getX(),getY(),Outline_y+8,Outline_y+150);
...
//					System.out.println("b ="+now_bit);
					...
					PFitness_JLabel.setText("<html><div style='text-align: center;'>"+Generation[i].X[best].BitInfo[now_bit]+"<br>"+Generation[i].X[best].Bit[now_bit]+"  "+Generation[i].X[best].Probability[now_bit]+"</div></html> ");
			...
			else if(System_Mode == 2)
			{
				if(e.getSource()==Answer_JLabel)
				{
					...
					if(Show_P == 0)
					{
						...
						PFitness_JLabel.setText("<html><div style='text-align: center;'>"+df0.format(Map[Now_Generation].Probability[col][row])+"<br> Limit :"+Map[Now_Generation].limit[row][col]+"</div></html> ");
						PFitness_JLabel.setVisible(true);
					}
					else
					{
										...
										for(int ij=c-6; ij<c+6; ij++)
										{
											for(int ik=r-6; ik<r+6; ik++)
											{
												if(ij>=0 && ik>=0 && ij<temp_Map.col && ik<temp_Map.row)
												{
													...
													{
														double distance=Math.sqrt((ij-c)*(ij-c)+(ik-r)*(ik-r));
...
//					System.out.println(Map[Now_Generation].Probability[row][col]);
					Text_Fitness.setText("f("+(row+1)+","+(col+1)+")="+Map[Now_Generation].Probability[row][col]);
				}
			}
		}
		//拖移滑鼠
...
//			Button_Confirm.setVisible(false);
			...
			{
				Show.Speed*=4;
				if(Show.Speed>4096)Show.Speed=16;
				int_Speed=(int)Math.sqrt(Show.Speed);
				Text_Speed.setText(""+(int_Speed/4));
			...
			{
				Show.Speed/=4;
				if(Show.Speed<16)Show.Speed=4096;
				int_Speed=(int)Math.sqrt(Show.Speed);
				Text_Speed.setText(""+(int_Speed/4));
				Show.int_Speed=(int)Math.sqrt(Show.Speed);
				Show.Text_Speed.setText(""+Show.int_Speed);
...
//		if(!Show.Overlapping_ToggleButton2.isSelected() )
		...
		try
		{
			...
			{
				//讀取地圖
				if(temp_Str.length()>=Should_S_Str[4].length() && temp_Str.substring(0,Should_S_Str[4].length()).compareToIgnoreCase(Should_S_Str[4])==0)
				{
...
//											System.out.println("ik="+ik);
											if(ij>=0 && ik>=0 && ij<temp_c && ik<temp_r)
											{
												...
												{
													double distance=Math.sqrt((ij-j)*(ij-j)+(ik-k)*(ik-k));
													//if(distance<=5)
														temp*=(1-(double)1/distance);													
...
//					System.out.println("----");
				...
				//讀取粒子紀錄檔Particle
				else if(temp_Str.length()>=Should_S_Str[2].length() && temp_Str.substring(0,Should_S_Str[2].length()).compareToIgnoreCase(Should_S_Str[2])==0)
				{
					if(Show.System_Mode != 1)
						Show.Mode_Change = 1;
					Show.System_Mode = 1;
...
//						System.out.println("GBest="+Show.Generation[i].CBest);
						i++;
					}
					Show.Total_Generation=i-1;
//					System.out.println("Tol_Gen="+Show.Total_Generation);
